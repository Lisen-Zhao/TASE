...
def PSF_result(data,dataname,aim_date,ps_index):
    ...
    for i in ps_index:
    #     if np.max(aimdata.loc[i, 'season']) == np.max(aimdata.loc[aim_date, 'season']) and np.max(aimdata.loc[i, 'weekday']) == np.max(aimdata.loc[aim_date, 'weekday']):
    #
    #         onedayload = np.array(data_mm.loc[i, 'load'].values.flatten())
    #
    #         onedayload = onedayload.reshape(48, 1)
