...
print(mape_values)
pd.DataFrame(mape_values, index=ok_list).to_csv('2.82tas.csv')
# torch.save(model,'the_model.tar')
pd.DataFrame(pred.reshape(-1,24), columns=[str(x) + 'h' for x in range(1,25)], index=aim_list).to_csv('pred_2.82value.csv')
